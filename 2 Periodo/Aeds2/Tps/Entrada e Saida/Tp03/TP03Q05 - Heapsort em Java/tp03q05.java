...
class Lista3 {
    ...
    public void ordenaSelecao() {
        // ordenacao por selecao busca o menor numero e ordena eles por causa disso
   //Alterar o vetor ignorando a posicao zero
    Serie6 [] tmp = new Serie6[n+1];
    for(int i = 0; i < n; i++){
    tmp[i+1] = array[i];
}
array = tmp;
...
public void construir(int tamHeap){
    for(int i = tamHeap; ((i > 1) && ((array[i].getFormat().compareTo(array[i/2].getFormat()) > 0)|| 
    ((array[i].getFormat().compareTo(array[i/2].getFormat()) == 0) && 
    (array[i].getName().compareTo(array[i/2].getName())> 0)))); i /= 2){
        swap(i, i/2);
    }
    }
...
public void reconstruir(int tamHeap){
    ...
    while(i <= (tamHeap/2)){
        ...
        if((array[filho].getFormat().compareTo(array[i].getFormat()) > 0) || 
        ((array[filho].getFormat().compareTo(array[i].getFormat()) == 0) && 
        (array[filho].getName().compareTo(array[i].getName())> 0))){
            swap(i, filho);
            i = filho;
        }else{
            i = tamHeap;
...
public int getMaiorFilho(int i, int tamHeap){
    ...
    ((array[2*i].getFormat().compareTo(array[2*i+1].getFormat()) == 0) && 
