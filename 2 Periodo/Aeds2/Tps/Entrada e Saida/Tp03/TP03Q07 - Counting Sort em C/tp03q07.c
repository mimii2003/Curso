...
void ler_serie(Serie *serie, char *html) {
    ...
    ptr = strstr(ptr, "<td");
    sscanf(trim(texto), "%d", &serie->num_temporadas);
...
void Desempatar() {
        ...
        for (int j = (i + 1); j < n; j++){
            comparacoes++;
            if (array[j].num_temporadas< array[menor].num_temporadas) {
                comparacoes++;
            menor = j;
        }
        int teste2;
        teste2 = strcmp(array[j].nome, array[menor].nome);
        if(array[j].num_temporadas == array[menor].num_temporadas){
            movimentacoes++;
            if(teste2<0){
                comparacoes++;
...
int main() {
    ...
    FILE *p = fopen ("matricula_countingsort.txt", "w+");
