...
class Serie6{
    ...
    int episodes, int radix){
        ...
        this.radix = ((episodes*1000) + seasons);
    }
    ...
    public int getRadix(){ 
        this.radix = this.episodes * 1000 + this.seasons;
        return this.radix; 
    }
...
class Lista3 {
    ...
    public int Maximo()
    {
        int mx = array[0].getRadix();
        for (int i = 1; i < n; i++){
        comparacoes++;
            if (array[i].getRadix() > mx){
                comparacoes++;
                movimentacoes++;
                mx = array[i].getRadix();}}
    ...
    {
        int exp =1;
        Serie6 output[] = new Serie6[n]; 
        int i;
        int aux[] = new int[10];
        Arrays.fill(aux, 0);
        for (i = 0; i < n; i++){
            comparacoes++;
            aux[(array[i].getRadix() / exp) % 10]++;}
        ...
        for (i = n - 1; i >= 0; i--) {
            comparacoes++;
            movimentacoes++;
            output[aux[(array[i].getRadix() / exp) % 10] - 1] = array[i];
            aux[(array[i].getRadix() / exp) % 10]--;
        }
        for (i = 0; i < n; i++){
            comparacoes++;
    ...
    void radixsort()
    {
        ...
        for (int exp = 1; max / exp > 0; exp *= 10)
            countSort();
            // desemapatr as variaveis iguais
            Desempatar();
    }
    //felipe disse pra desempatar com o insercao
    public void Desempatar() {
        ...
        int menor = aux;
            for (int i = (aux + 1); i < n; i++) {
                comparacoes++;
                if (array[i].getRadix() < (array[menor].getRadix())) {
                    menor = i;
                    comparacoes++;
                }
                if (array[i].getRadix() == (array[menor].getRadix())) {
                    movimentacoes++;
                    if(array[i].getName().compareTo(array[menor].getName()) < 0) {
                        comparacoes++;
                        menor = i;
                    }
                }
